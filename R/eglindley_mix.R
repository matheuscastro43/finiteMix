eglindley_mix = function(data, g, lim.em = 100, criteria = "dif.psi", plot.it = TRUE, empirical = FALSE,
    ...
    while(T){
      ...
      Q = function(param){
        ...
        Qj = function(j){
          aux = 0
          for(i in 1:n){
            aux = aux + Wij[i,j] * log(pi[j] * dglindley(data[i], alphast[j], betast[j],
                                                         gammast[j]))
          }
          return(aux)
        }
      ...
      grQ = function(param){
        ...
        for(i in 1:n){
          gr1 = gr1 + (Wij[i,] * log(data[i]) + Wij[i,]/
                         (alphast + gammast * data[i]) - log(betast) * Wij[i,] -
                         digamma(alphast + 1) * Wij[i, ])
          gr2 = gr2 + ((Wij[i, ] * data[i])/betast^2 - alphast/betast * Wij[i,] -
                         gammast/(betast * gammast + 1) * Wij[i, ])
  ...
  if(plot.it == TRUE){
    ...
    for(i in 1:g){
      if(alphas[i] >= 1){
        modal[i] = max(dglindley(c((alphas[i]-1)*betas[i], (alphas[i])*betas[i]),
